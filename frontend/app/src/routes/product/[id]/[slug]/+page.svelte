<script lang="ts">
    ...
    import { getProduct, getCategories, type Product, type Category } from '$lib/api';
    import SearchModal from "$lib/components/SearchModal.svelte";
    import { goto } from '$app/navigation';
    import { error } from '@sveltejs/kit';

    export let data: { params: { id: string; slug: string } };
    ...
    onMount(async () => {
        try {
            const [productData, categoriesData] = await Promise.all([
                getProduct(data.params.id, fetch),
                getCategories(fetch)
            ]);
    ...
    // Обработка события из модалки
    async function handleApply(event: CustomEvent<{ params?: URLSearchParams }>) {
        const params = event.detail.params;

        // По желанию сохраняем фильтры в URL (для ссылки/refresh)
...
{:else if errorMessage}
    <div class="error">
        <h2>Ошибка</h2>
        <p>{errorMessage}</p>
        <button on:click={() => goto('/')}>Вернуться на главную</button>
    </div>
{:else if product}
